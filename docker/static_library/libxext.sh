...
{
  git clone https://gitlab.freedesktop.org/xorg/lib/libxext.git libxext
} 2>&1 | tee "$LOG_DIR/libxext-01-clone.log"

STATUS=${PIPESTATUS[0]}
if [ "$STATUS" -ne 0 ]; then
  cat "$LOG_DIR/libxext-01-clone.log"
  echo "ERROR - libxext - 01/05 - Clone"
  exit 1
fi
...
if [ "$STATUS" -ne 0 ]; then
  ...
  echo "ERROR - libxext - 02/05 - Clone"
  exit 1
fi
...
if [ "$STATUS" -ne 0 ]; then
  ...
  echo "ERROR - libxext - 03/05 - Configure"
  exit 1
fi
...
if [ "$STATUS" -ne 0 ]; then
  ...
  echo "ERROR - libxext - 04/05 - Compile"
  exit 1
fi
...
if [ "$STATUS" -ne 0 ]; then
  ...
  echo "ERROR - libxext - 05/05 - Install"
  exit 1
fi
